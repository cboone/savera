...
- **The audio output port is 32-bit only, so `process()` requires `data32`.** In the pinned CLAP 1.2.10, `clap/ext/audio-ports.h` makes 32-bit support mandatory for host and plugin and 64-bit optional behind `CLAP_AUDIO_PORT_SUPPORTS_64BITS`. The port does not set that flag, so a host may not hand it a `data64` buffer, and refusing a buffer without `data32` is correct. Do not suggest handling `data64` unless the same change sets the flag.
- **Only note events take a `-1` port wildcard; raw MIDI events must name port 0.** CLAP's `(port, channel, key, note_id)` wildcard rule covers note, note-expression and modulation addresses. `clap_event_midi_t.port_index` is a `uint16_t` with no wildcard value, so a MIDI event for any port other than the single note input's index 0 is ignored. Do not suggest accepting `0xFFFF` or `-1` there.
