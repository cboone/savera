...
Logic discovery failed on 2026-09-16: Logic Pro 12.3.1 on macOS 26.6.2 did not list `Catamount: Savera` in the plug-ins menu after restart with installed build `1de6a7f`. [Issue #5](https://github.com/cboone/savera/issues/5) tracks the failure. The installed AU still passes strict signature verification, exports its factory entry point, and declares the expected instrument metadata. The local Audio Component capability cache contains its `aumu` / `Svra` / `Ctmn` record with a stereo output and MIDI support. Logic's Plug-in Manager presence, validation status and enabled state remain pending; no wrapper defect has been established. System registration logs could not be read because macOS denied access to the local log store.
...
- **Result:** passed with weaker confirmation. 2026-09-16, installed build `1de6a7f`: both source/installed binary hashes match the table above, signatures pass, and both markers report the same clean build. The user confirmed restarting Logic Pro 12.3.1 on macOS 26.6.2 after installation. Running-host provenance is unavailable because the instrument does not appear in the menu.
...
- **Result:** failed. 2026-09-16, Logic Pro 12.3.1 on macOS 26.6.2, installed build `1de6a7f`: the user reports no `Catamount: Savera` in the plug-ins menu after restart. Instrument insertion is unverified. Plug-in Manager presence, validation status and enabled state are the next observations; see [issue #5](https://github.com/cboone/savera/issues/5).
